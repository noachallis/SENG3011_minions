...
export const getPolygonLabel = (d : any, dateData : any, language : string) => {
    ...
    if (country){
        ...
        ${getWord('pop_vacced', language)}: <i>${(country.properties.people_fully_vaccinated/country.properties.population * 100).toFixed(0)}%</i>
        `)
    } else {
        return (`
        <b>${d.ADMIN} (${d.ISO_A3}):</b> <br />
        ${getWord('total_cases', language)}: <i>0</i><br/>
        ${getWord('pop_vacced', language)}: <i>0%</i>
        `)
    }
}
